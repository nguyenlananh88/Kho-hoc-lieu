...
export default function AdminDashboard({
  ...
  // Custom Confirm Modal state to bypass window.confirm iframe block
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoginError(`Không thể kết nối với máy chủ backend. Lỗi: ${err.message || err}`);
    } finally {
  ...
  const handleDeleteAdminAccount = (delUsername: string) => {
    setConfirmDeleteAction({
      message: `Bạn có chắc muốn thu hồi quyền truy cập (Xóa tài khoản) của "${delUsername}" không?`,
      onConfirm: async () => {
        try {
  ...
  const handleDeleteOrder = (orderId: string) => {
    setConfirmDeleteAction({
      message: "Thầy cô có chắc chắn xóa vĩnh viễn hóa đơn này khỏi hệ thống?",
      onConfirm: async () => {
  ...
  const handleDeleteProduct = (prodId: string) => {
    setConfirmDeleteAction({
      message: "Bộ GDPT 2018: Thầy cô chắc chắn muốn xóa học liệu này?",
      onConfirm: async () => {
        try {
  ...
  const handleDeleteInitiative = (initId: string) => {
    setConfirmDeleteAction({
      message: "Thầy cô chắc chắn muốn xóa sáng kiến kinh nghiệm này?",
      onConfirm: async () => {
        try {
          ...
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-150 animate-scale-up text-left">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                className="px-4 py-2 text-xs font-bold text-white bg-red-650 hover:bg-red-600 rounded-xl shadow-md transition-all cursor-pointer"
              >
                Xác nhận
              </button>
            </div>
