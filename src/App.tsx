...
export default function App() {
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("persistent_active_tab") || "trang-chu";
    }
    return "trang-chu";
  });
  ...
  // Custom Confirmation Dialog to bypass window.confirm iframe blocks
  ...
  const [selectedSubject, setSelectedSubject] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("persistent_selected_subject") || "Tất cả";
    }
    return "Tất cả";
  });
  const [selectedGrade, setSelectedGrade] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("persistent_selected_grade") || "Tất cả";
    }
    return "Tất cả";
  });
  const [selectedType, setSelectedType] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("persistent_selected_type") || "Tất cả";
    }
    return "Tất cả";
  });
  ...
  const [isAdmin, setIsAdmin] = useState<boolean>(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("admin-token") === "admin-secret-token";
    }
    return false;
  });
  ...
  const [bankSettings, setBankSettings] = useState<any>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("backup_bank");
      if (saved) {
        try { return JSON.parse(saved); } catch (_) {}
      }
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data && typeof data === "object") {
          ...
          try {
            localStorage.setItem("backup_bank", JSON.stringify(data));
          } catch (_) {}
        }
      }
    } catch (err) {
  ...
  const [adminSubTab, setAdminSubTab] = useState<"dashboard" | "products" | "orders" | "feedbacks" | "settings">(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("persistent_admin_sub_tab");
      if (saved && ["dashboard", "products", "orders", "feedbacks", "settings"].includes(saved)) {
        return saved as any;
      }
    }
    return "dashboard";
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (fallbackStr) {
          setProducts(JSON.parse(fallbackStr));
        }
      }
    } catch (err) {
  ...
  const handleAddSubject = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/admin/subjects", {
        ...
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${adminToken || localStorage.getItem("admin-token")}`
        },
  ...
  const handleDeleteSubject = (name: string) => {
    setConfirmDialog({
      message: `Bạn có chắc chắn muốn xóa môn học "${name}" không?`,
      onConfirm: async () => {
        try {
  ...
  // --- CLIENT & ADMIN STATE PERSISTENCE EFFECTS ---
  useEffect(() => {
    localStorage.setItem("persistent_active_tab", activeTab);
  }, [activeTab]);
  ...
  useEffect(() => {
    localStorage.setItem("persistent_selected_subject", selectedSubject);
  }, [selectedSubject]);

  useEffect(() => {
    localStorage.setItem("persistent_selected_grade", selectedGrade);
  }, [selectedGrade]);

  useEffect(() => {
    localStorage.setItem("persistent_selected_type", selectedType);
  }, [selectedType]);

  useEffect(() => {
    localStorage.setItem("persistent_admin_sub_tab", adminSubTab);
  }, [adminSubTab]);
  ...
  // Load Admin Token from local storage on init
  useEffect(() => {
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const localBackupStr = localStorage.getItem("backup_products");
        let localBackupList: Product[] = [];
        if (localBackupStr) {
          try {
        ...
        if (productFormMode === "create") {
          ...
          const idx = localBackupList.findIndex(item => item.id === selectedProductId);
          if (idx !== -1) {
        ...
        localStorage.setItem("backup_products", JSON.stringify(localBackupList));
  ...
  const handleDeleteInit = (id: string) => {
    setConfirmDialog({
      message: "Admin: Thầy cô chắc chắn muốn xóa sáng kiến này khỏi hệ thống?",
      onConfirm: async () => {
        try {
  ...
  const handleDeleteGame = (id: string) => {
    setConfirmDialog({
      message: "Admin: Thầy cô chắc chắn muốn xóa trò chơi này khỏi hệ thống?",
      onConfirm: async () => {
        try {
                          ...
                          <div 
                            ...
                            onClick={() => {
                              const mappedProduct = {
                                ...
                                id: g.id,
                                title: g.title,
                                price: g.isPaid ? (g.salePrice || g.price || 0) : 0,
                                originalPrice: g.isPaid ? (g.price || 0) : 0,
                                description: g.desc,
                                subject: g.subject || g.category || "Tin học AI",
                                grade: g.grade || null,
                                type: "Trò chơi tương tác AI",
                                rating: 5.0,
                          ...
                          >
                            ...
                            {isAdmin && (
                              <div className="absolute top-2 right-2 z-20 flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                                <button
                                  ...
                                  className="bg-indigo-600 hover:bg-indigo-700 text-white p-1.5 rounded-full shadow-lg transition-all transform hover:scale-110 active:scale-95 cursor-pointer flex items-center justify-center"
                            ...
                            <div className="relative h-28 sm:h-32 w-full bg-slate-50 overflow-hidden">
                              {g.image ? (
                                ...
                                <div className={`w-full h-full bg-gradient-to-r ${currentGradient}`}></div>
                              )}
                              ...
                              <span className="absolute top-1.5 left-1.5 bg-slate-950/90 backdrop-blur-sm text-white text-[8px] sm:text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full select-none">
                                {g.subject || g.category || "Tin học AI"}
                              </span>
                            ...
                            <div className="p-3 sm:p-3.5 flex-grow flex flex-col justify-between space-y-2 sm:space-y-2.5">
                              <div className="space-y-1">
                                <span className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1">
                                  <Sparkles className="w-2.5 h-2.5 text-indigo-500" /> Trò chơi học tập
                                </span>
                              ...
                              <div className="pt-2 border-t border-slate-100 flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center justify-between">
                                  <div>
                                    {!g.isPaid ? (
                                      ...
                                      <div className="space-y-0.5 leading-none">
                                        <p className="text-slate-800 font-extrabold text-xs sm:text-xs md:text-sm">
                                          {(g.salePrice || g.price || 0).toLocaleString("vi-VN")}đ
                                        </p>
                                        {g.price > (g.salePrice || 0) && (
                                          <p className="text-[8.5px] sm:text-[9px] text-slate-400 line-through">
                                            {g.price.toLocaleString("vi-VN")}đ
                                          </p>
                                        )}
                                      </div>
                                    )}
                                  ...
                                  <button
                                    ...
                                    className="py-1 px-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-[9px] sm:text-[10px] font-extrabold transition-all flex items-center gap-0.5 cursor-pointer"
                                  >
                                    Chơi thử <ChevronRight className="w-2.5 h-2.5" />
                                  </button>
                                </div>
                                ...
                                <div className="flex gap-1.5 w-full">
                                  {g.isPaid ? (
                                    <button
                                      ...
                                      className="flex-1 py-1 px-2 sm:py-1.5 sm:px-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-[9px] sm:text-[10px] font-extrabold transition-all flex items-center justify-center gap-1 cursor-pointer"
                                    >
                                      <ShoppingCart className="w-3 h-3" /> Mua tệp game
                                    </button>
                                  ) : (
                                    <button
                                      onClick={() => triggerDownload(g)}
                                      className="flex-1 py-1 px-2 sm:py-1.5 sm:px-2.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 rounded-lg text-[9px] sm:text-[10px] font-extrabold transition-all flex items-center justify-center gap-1 cursor-pointer border border-emerald-150"
                                    >
                                      <Download className="w-3 h-3" /> Tải về tệp
                                    </button>
                                  )}
                                </div>
                              </div>
...
WHERE id = '${finalId}';`;
    ...
    {confirmDialog && (
      <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()}>
        <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-150 animate-scale-up text-left">
          <div className="flex items-center gap-3 mb-4">
            <div className="bg-red-50 text-red-650 p-2.5 rounded-2xl">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-400">Hành động này không thể hoàn tác</p>
            </div>
          </div>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button
              ...
              className="px-4 py-2 text-xs font-bold text-white bg-red-600 hover:bg-red-500 rounded-xl shadow-md transition-all cursor-pointer"
            >
              Xác nhận
            </button>
          </div>
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (localBackupStr) {
          try {
            const localBackup = JSON.parse(localBackupStr);
            const filteredBackup = localBackup.filter((bp: any) => bp.id !== product.id);
            localStorage.setItem("backup_products", JSON.stringify(filteredBackup));
          } catch (_) {}
        }
        onDelete();
      }
    ...
    >
      ...
      <div className="p-3 sm:p-3.5 flex-grow flex flex-col justify-between space-y-2">
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1">
              <Layers className="w-2.5 h-2.5 text-indigo-500" /> Sáng kiến kinh nghiệm
            </span>
          ...
          <p className="text-[9.5px] sm:text-[10px] font-bold text-slate-450 italic leading-none">
            Tác giả: {item.author}
          </p>
        ...
        <div className="pt-2 border-t border-slate-100 flex items-center justify-between" onClick={(e) => e.stopPropagation()}>
          <div>
            {isFree ? (
              ...
              <div className="space-y-0.5 leading-none">
                <p className="text-slate-800 font-extrabold text-xs sm:text-xs md:text-sm">{item.price.toLocaleString("vi-VN")}đ</p>
                <p className="text-[7.5px] sm:text-[8px] text-slate-400 font-bold uppercase tracking-wider">Kèm phụ lục Sở</p>
              </div>
            )}
          </div>
