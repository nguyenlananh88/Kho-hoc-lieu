...
async function initializeSupabase() {
  ...
  try {
    ...
    // Seed Banner Settings if empty
    try {
      const { data: existingBanner, error: bannerErr } = await supabase
      ...
      if (bannerErr) {
        handleSupabaseError(bannerErr, "banner_settings");
      } else {
        if (!existingBanner || existingBanner.length === 0) {
          console.log("[Supabase] Empty banner_settings detected. Auto-seeding default banner...");
          const localBanner = readData(BANNER_FILE, DEFAULT_BANNER);
          const { error: seedErr } = await supabase.from("banner_settings").insert({
            id: "current",
            background_image: localBanner.backgroundImage || "",
            badge_text: localBanner.badgeText || "",
            title_1: localBanner.title1 || "",
            title_2: localBanner.title2 || "",
            description: localBanner.description || "",
            admin_panel1_title: localBanner.adminPanel1Title || "",
            admin_panel1_desc: localBanner.adminPanel1Desc || "",
            admin_panel2_title: localBanner.adminPanel2Title || "",
            admin_panel2_desc: localBanner.adminPanel2Desc || "",
            admin_panel3_title: localBanner.adminPanel3Title || "",
            promo_badge: localBanner.promoBadge || "",
            promo_title1: localBanner.promoTitle1 || "",
            promo_title2: localBanner.promoTitle2 || "",
            promo_desc: localBanner.promoDesc || "",
            promo_foot: localBanner.promoFoot || "",
            promo_btn: localBanner.promoBtn || "",
            promo_enabled: localBanner.promoEnabled === true || String(localBanner.promoEnabled) === "true"
          });
          if (seedErr) {
            console.error("[Supabase] Auto-seeding banner_settings failed:", seedErr.message);
          } else {
            console.log("[Supabase] Auto-seeded banner_settings successfully!");
          }
        }
      }
    } catch (e: any) {
      handleSupabaseError(e, "banner_settings");
    }

    // Seed Bank Settings if empty
    try {
      const { data: existingBank, error: bankErr } = await supabase
        .from("bank_settings")
        .select("id")
        .eq("id", "current")
        .limit(1);
      ...
      if (bankErr) {
        handleSupabaseError(bankErr, "bank_settings");
      } else {
        if (!existingBank || existingBank.length === 0) {
          console.log("[Supabase] Empty bank_settings detected. Auto-seeding default bank settings...");
          const localBank = readData(BANK_FILE, DEFAULT_BANK_SETTINGS);
          const { error: seedErr } = await supabase.from("bank_settings").insert({
            id: "current",
            bank_name: localBank.bankName || "",
            account_number: localBank.accountNumber || "",
            account_holder: localBank.accountHolder || "",
            memo_template: localBank.memoTemplate || "",
            is_enabled: localBank.isEnabled === true || String(localBank.isEnabled) === "true"
          });
          if (seedErr) {
            console.error("[Supabase] Auto-seeding bank_settings failed:", seedErr.message);
          } else {
            console.log("[Supabase] Auto-seeded bank_settings successfully!");
          }
        }
      }
    } catch (e: any) {
      handleSupabaseError(e, "bank_settings");
    }
  ...
  if (SUPABASE_URL && SUPABASE_KEY) {
    try {
      const checkTable = async (tableName: string) => {
        try {
          // subjects table uses 'name', admin_accounts uses 'username', others use 'id'
          const col = tableName === "subjects" ? "name" : (tableName === "admin_accounts" ? "username" : "id");
      ...
      const gamesCheck = await checkTable("games");
      const subjectsCheck = await checkTable("subjects");
      const adminAccountsCheck = await checkTable("admin_accounts");
      const bannerSettingsCheck = await checkTable("banner_settings");
      const bankSettingsCheck = await checkTable("bank_settings");
      ...
      status.tables.games = gamesCheck;
      status.tables.subjects = subjectsCheck;
      status.tables.admin_accounts = adminAccountsCheck;
      status.tables.banner_settings = bannerSettingsCheck;
      status.tables.bank_settings = bankSettingsCheck;

      const allExist = productsCheck.exists && 
                       initiativesCheck.exists && 
                       ordersCheck.exists && 
                       feedbacksCheck.exists &&
                       gamesCheck.exists &&
                       subjectsCheck.exists &&
                       adminAccountsCheck.exists &&
...
app.get("/api/bank", async (req, res) => {
  ...
  const mergedData = { ...DEFAULT_BANK_SETTINGS, ...localData };
  ...
  if (SUPABASE_URL && SUPABASE_KEY && !useSupabaseFallback) {
    try {
      ...
      if (!error && data) {
        return res.json({
          ...mergedData,
          bankName: data.bank_name || mergedData.bankName,
          accountNumber: data.account_number || mergedData.accountNumber,
          accountHolder: data.account_holder || mergedData.accountHolder,
          memoTemplate: data.memo_template || mergedData.memoTemplate,
          isEnabled: data.is_enabled !== undefined ? (data.is_enabled === true || data.is_enabled === "true" || String(data.is_enabled) === "true") : mergedData.isEnabled
        });
      }
    } catch (e: any) {
      console.warn("[Supabase] Failed to fetch bank_settings:", e.message || e);
    }
  }
...
app.post("/api/admin/bank", requireAdmin, async (req, res) => {
    ...
    try {
      const { error } = await supabase
        ...
        .upsert({
          ...
          memo_template: settings.memoTemplate || "",
          is_enabled: settings.isEnabled === true || settings.isEnabled === "true" || String(settings.isEnabled) === "true",
          updated_at: new Date().toISOString()
        });
      if (error) {
        console.warn("[Supabase] Failed to upsert bank_settings:", error.message);
      }
    } catch (e: any) {
      console.warn("[Supabase] Exception in save bank_settings:", e.message || e);
    }
  }
...
// 6. Offline / Local Simulation Mode for Lesson Planner & Image Analysis (Removed external AI dependencies to avoid errors)
app.post("/api/gemini/generate-lesson-plan", async (req, res) => {
  ...
  // Return a beautiful simulated lesson plan without calling external Google Gen AI APIs
  const offlineOutput = `# GIÁO ÁN MINH HỌA GDPT 2018 (Chế độ Ngoại tuyến)
- **Môn học**: ${subject || "Tin học / Công nghệ"}
- **Khối lớp**: ${grade || "Khối trung học"}
- **Chủ đề**: ${topic}
- **Mục tiêu**: ${objectives || "Đạt chuẩn đầu ra môn học và phát triển năng lực số"}

## I. MỤC TIÊU BÀI HỌC
1. **Kiến thức**: Học sinh nắm vững cấu trúc cơ bản và nguyên lý vận hành của chủ đề bài học.
2. **Năng lực số**: Sử dụng phần mềm học liệu tương tác trực quan để thiết kế mô hình nhị phân/thuật toán.
3. **Phẩm chất**: Rèn luyện tư duy phản biện, làm việc nhóm và trách nhiệm công dân số.

## II. THIẾT BỊ DẠY HỌC & HỌC LIỆU SỐ
- Máy tính kết nối Internet mạng nội bộ, máy chiếu LCD.
- Học liệu số EduShop AI, phần mềm lập trình mô phỏng khối lệnh tương tác trực tuyến.

## III. TIẾN TRÌNH DẠY HỌC
### 1. Hoạt động Khởi động (5 phút)
- Giáo viên cho học sinh quan sát một sơ đồ số hóa. Học sinh dự đoán chức năng của từng thành phần.
### 2. Hoạt động Hình thành kiến thức (15 phút)
- Hướng dẫn học sinh tra cứu và phân tích các bộ phận linh kiện điện tử hoặc bài thực hành thông qua hệ thống học liệu Edushop AI.
### 3. Hoạt động Luyện tập (15 phút)
- Học sinh thực hành nhóm lập trình điều khiển giả lập cơ chế tự động hóa theo kịch bản mẫu.
### 4. Hoạt động Vận dụng (10 phút)
- Liên hệ ứng dụng thực tế của kiến thức bài học trong đời sống xã hội số hóa hiện đại.`;

  res.json({ output: offlineOutput });
});
...
app.post("/api/gemini/analyze-image", async (req, res) => {
  ...
  // Directly return a clear simulated error to trigger the robust frontend educational simulator!
  // This avoids calling Google Gen AI endpoints and ensures full operability without any API Key.
  res.status(500).json({ error: "Tính năng phân tích trực tuyến tạm thời tắt. Hệ thống tự động chuyển sang phân tích cục bộ." });
});
