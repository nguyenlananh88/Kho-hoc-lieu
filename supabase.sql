...
-- 8. Create Banner Settings table
...
DROP POLICY IF EXISTS "Allow public read access to admin_accounts" ON admin_accounts;
DROP POLICY IF EXISTS "Allow public read access to banner_settings" ON banner_settings;
DROP POLICY IF EXISTS "Allow public read access to bank_settings" ON bank_settings;
...
CREATE POLICY "Allow public read access to bank_settings" ON bank_settings FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow public full access to products" ON products;
DROP POLICY IF EXISTS "Allow public full access to initiatives" ON initiatives;
DROP POLICY IF EXISTS "Allow public full access to orders" ON orders;
DROP POLICY IF EXISTS "Allow public full access to feedbacks" ON feedbacks;
DROP POLICY IF EXISTS "Allow public full access to games" ON games;
DROP POLICY IF EXISTS "Allow public full access to subjects" ON subjects;
DROP POLICY IF EXISTS "Allow public full access to admin_accounts" ON admin_accounts;
DROP POLICY IF EXISTS "Allow public full access to banner_settings" ON banner_settings;
DROP POLICY IF EXISTS "Allow public full access to bank_settings" ON bank_settings;
...
CREATE POLICY "Allow public full access to bank_settings" ON bank_settings FOR ALL USING (true) WITH CHECK (true);

-- API/Service-role full controls (Required for server-side management)
DROP POLICY IF EXISTS "Allow full access for service_role to games" ON games;
DROP POLICY IF EXISTS "Allow full access for service_role" ON products;
DROP POLICY IF EXISTS "Allow full access for service_role" ON initiatives;
DROP POLICY IF EXISTS "Allow full access for service_role" ON orders;
DROP POLICY IF EXISTS "Allow full access for service_role" ON feedbacks;
DROP POLICY IF EXISTS "Allow full access for service_role to subjects" ON subjects;
DROP POLICY IF EXISTS "Allow full access for service_role to admin_accounts" ON admin_accounts;
DROP POLICY IF EXISTS "Allow full access for service_role to banner_settings" ON banner_settings;
DROP POLICY IF EXISTS "Allow full access for service_role to bank_settings" ON bank_settings;
...
CREATE POLICY "Allow full access for service_role to banner_settings" ON banner_settings FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow full access for service_role to bank_settings" ON bank_settings FOR ALL USING (true) WITH CHECK (true);
